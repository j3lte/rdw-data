 ...
 */
export interface Gebieden1_ResponseData {
  /**
   * ### 01_NAAM
   *
   * **Type**: Text
   ...
   * **Type**: MultiPolygon
   */
  the_geom?: { type: "MultiPolygon"; coordinates: number[][][][] };
  /**
   * ### UsageId
   *
   * **Type**: Text
...
export const Fields: {
  ...
  TheGeom: FieldObject<DataType.MultiPolygon>;
...
/**
 * Dataset metadata for **Gebieden1**: the available field names, the
...
export const RDWQuery = (
  auth: AuthOpts = {},
  opts: Options = {},
): SodaQuery<Gebieden1_ResponseData> =>
  createQueryWithDataset<Gebieden1_ResponseData>(Info.domain, Info.dataset, auth, {
    ...opts,
 ...
 * - {@link Gebieden1.RDWQuery} — a typed {@link SodaQuery} builder
 * - {@link Gebieden1.Fields} — the queryable field definitions
 * - {@link Gebieden1.Info} — dataset metadata
 *
 * **Category:** Parkeren
 *
 * **Tags:** parkeren
 ...
 * @example
 * ```ts
 * const { data } = await Gebieden1.RDWQuery()
 *   .where(Where.like(Gebieden1.Fields.N01Naam, "some_value"))
 *   .limit(10)
 *   .execute();
 * ```
 */
export const Gebieden1 = {
  RDWQuery,
  Fields,
  Info,
};
