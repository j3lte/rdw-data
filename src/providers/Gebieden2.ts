 ...
 */
export interface Gebieden2_ResponseData {
  /**
   * ### 01_NAAM
   *
   * **Type**: Text
...
/**
 * Dataset metadata for **Gebieden2**: the available field names, the
...
export const RDWQuery = (
  auth: AuthOpts = {},
  opts: Options = {},
): SodaQuery<Gebieden2_ResponseData> =>
  createQueryWithDataset<Gebieden2_ResponseData>(Info.domain, Info.dataset, auth, {
    ...opts,
 ...
 * - {@link Gebieden2.RDWQuery} — a typed {@link SodaQuery} builder
 * - {@link Gebieden2.Fields} — the queryable field definitions
 * - {@link Gebieden2.Info} — dataset metadata
 *
 * **Category:** Parkeren
 *
 * **Tags:** parkeren
 ...
 * @example
 * ```ts
 * const { data } = await Gebieden2.RDWQuery()
 *   .where(Where.like(Gebieden2.Fields.N01Naam, "some_value"))
 *   .limit(10)
 *   .execute();
 * ```
 */
export const Gebieden2 = {
  RDWQuery,
  Fields,
  Info,
};
